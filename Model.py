...
class FC_Deconv(torch.nn.Module):
    def __init__(self, features_in=108):
        super().__init__()
        self.fc = nn.Linear(features_in, 320)
        self.deconv1 = nn.ConvTranspose2d(in_channels=8, out_channels=4, kernel_size=4, stride=3)
        self.deconv2 = nn.ConvTranspose2d(in_channels=4, out_channels=2, kernel_size=4, stride=3)
        self.deconv3 = nn.ConvTranspose2d(in_channels=2, out_channels=1, kernel_size=4, stride=3)

    def forward(self, x):
        # x.shape: (features_in, )
        assert len(x.shape) == 1
        x = self.fc(x).reshape(1, 8, 10, 4)
        x = F.relu(x)
        x = self.deconv1(x)
        x = F.relu(x)
        x = self.deconv2(x)
